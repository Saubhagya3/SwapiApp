...
export default function Main({ allmovies }) {
  ...
  const saveToLocalStorage = (items) => {
    localStorage.setItem("swapi-app-favourites-saubhagya3", JSON.stringify(items))
  }
  ...
  return (
    <div>
      <Head>
        ...
        <Favourites favs={favourites} handleFavouritesClicks={removeFavouriteMovie} />
        <main className={styles.body}>
          {
            searchValue === "" ? 
            <MovieList movies={allmovies} handleFavouritesClicks={addFavouriteMovie} /> :
            <MovieList movies={movies} handleFavouritesClicks={addFavouriteMovie} />
          }
        </main>
    </div>
  );
